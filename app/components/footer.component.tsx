...
export function FooterComponent() {
  ...
  const onSubmit = async (data: { email: string }) => {
    try {
      ...
      toast({ title: "Email Added.", description: "We've added your email for news promotions.", status: "success", duration: 3000, isClosable: true });
    } catch (error) {
      toast({ title: "Internal Error", description: "An internal error occurred with the server.", status: "error", duration: 3000, isClosable: true });
    }
  };
  ...
  return (
    <footer className="flex w-full justify-between items-center h-[500px] max-md:flex-col-reverse">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <label htmlFor="email" className="font-normal block w-full max-md:text-center">
          Entrar em contato
        </label>
        <div className="mt-4 max-md:">
          <div className="input-wrapper">
            ...
            <input type="text" id="email" className="input" placeholder="info@gmail.com" {...register("email")} />
